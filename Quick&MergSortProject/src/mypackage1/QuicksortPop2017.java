...
public class QuicksortPop2017 {

	private County[] theArray; // ref to array theArray
	private int nElems; // number of data items
	// --------------------------------------------------------------

	public QuicksortPop2017(int max) // constructor
		{
	...
	public void recQuickSort(int left, int right) {
		...
		{
			int pivot = Integer.parseInt(theArray[right].getPopulation2017()); // rightmost item
			// partition range
	...
	public int partitionIt(int left, int right, int pivot) {
		...
		while (true) { // find bigger item
			while (Integer.parseInt(theArray[++leftPtr].getPopulation2017()) < pivot); // (nop)
					// find smaller item
			while (rightPtr > 0 && Integer.parseInt(theArray[--rightPtr].getPopulation2017()) > pivot); // (nop)
