...
public class QuicksortByName {

	private County[] theArray; // ref to array theArray
	private int nElems; // number of data items
	// --------------------------------------------------------------

	public QuicksortByName(int max) // constructor
		{
	...
	public void recQuickSort(int left, int right) {
		...
		{
			String pivot = theArray[right].getCountyName(); // rightmost item
			// partition range
	...
	// 2 values are compared in the array at each iteration
	// positive if first string is greater
	//negative if second string is greater
	// 0 if equal
	public int partitionIt(int left, int right, String pivot) {
		int leftPtr = left - 1; // left (after ++)
		int rightPtr = right; // right-1 (after --)
		while (true) { // find bigger item
			while (theArray[++leftPtr].getCountyName().compareTo(pivot) < 0); // (nop)
					// find smaller item
			while (rightPtr > 0 && theArray[--rightPtr].getCountyName().compareTo(pivot) > 0); // (nop)
