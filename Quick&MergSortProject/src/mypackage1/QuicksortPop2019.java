...
public class QuicksortPop2019 {

	private County[] theArray; // ref to array theArray
	private int nElems; // number of data items
	// --------------------------------------------------------------

	public QuicksortPop2019(int max) // constructor
		{
	...
	public void recQuickSort(int left, int right) {
		...
		{
			int pivot = Integer.parseInt(theArray[right].getPopulation2019()); // rightmost item
			// partition range
	...
	public int partitionIt(int left, int right, int pivot) {
		...
		while (true) { // find bigger item
			while (Integer.parseInt(theArray[++leftPtr].getPopulation2019()) < pivot); // (nop)
					// find smaller item
			while (rightPtr > 0 && Integer.parseInt(theArray[--rightPtr].getPopulation2019()) > pivot); // (nop)
