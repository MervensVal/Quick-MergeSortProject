...
public class Quicksort {
	...
	public void recQuickSort(int left, int right) {
		...
		{
			int pivot = Integer.parseInt(theArray[right].getPopulation2015()); // rightmost item
			// partition range
	...
	public int partitionIt(int left, int right, int pivot) {
		...
		while (true) { // find bigger item
			while (Integer.parseInt(theArray[++leftPtr].getPopulation2015()) < pivot); // (nop)
					// find smaller item
			while (rightPtr > 0 && Integer.parseInt(theArray[--rightPtr].getPopulation2015()) > pivot); // (nop)
